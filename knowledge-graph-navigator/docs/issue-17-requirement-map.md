...
| Linked data and schema.org-compatible metadata | Project nodes include schema.org-style scholarly metadata with identifiers and titles. |
...
| AI research recommendations | `recommendResearch()` ranks project suggestions from user interests, concept matches, citation signals, and reproducibility status. |
| Sidebar/digest/API readiness | `buildKnowledgeGraphPacket()` emits recommendation digests, navigation examples, and API route contracts. |
